# Python-free static server for Windows (PowerShell 5.1 compatible). Usage: powershell -File serve.ps1 <dir> [port]
...
@{port=$Port; pid=$PID} | ConvertTo-Json | Set-Content -Encoding utf8 (Join-Path $Root ".bwm\serve.json")
...
while ($listener.IsListening) {
  ...
  if ((Test-Path $file) -and $file.StartsWith($Root)) {
