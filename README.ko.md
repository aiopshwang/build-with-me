...
5. **걸음 지도.** 결과에서 입력까지 거꾸로 파고 그 순서를 뒤집은 것이 걸음 목록입니다. 범위 밖의 요구는 다투지 않고 "그건 2탄"에 적습니다. 보이는 것: `지도.md`의 체크리스트.
...
| 무엇 | 왜 있나 |
| --- | --- |
| `진행.md` | 진행 기록. 걸음이 끝날 때마다 "지금까지 된 것 / 다음" 두 줄. 이어서 할 때 가장 먼저 읽는 파일. |
| `지도.md` | 지도: 북극성, ✓가 붙는 걸음 목록, "그건 2탄", 두 번 막힌 자리. |
| `내-말로.md` | 걸음마다 학습자 자신의 말로 쓴 한 문장. 빈 걸음은 없습니다 — 나중에 이 앱을 남에게 설명할 때 읽는 파일. |
...
| `CLAUDE.md` / `AGENTS.md` | 다음 세션에 `진행.md`부터 읽고 "지난번엔 여기까지"라고 말하라는 한 줄이 덧붙습니다. |

`진행.md`·`지도.md`·`내-말로.md`에는 열쇠가 없습니다 — 그래서 시작 문서가 막힌 학습자에게 앞의 두 파일을 그대로 강사에게 보내라고 합니다. 템플릿: [`assets/`](skills/build-with-me/assets/).
...
새 세션이든 맥락이 정리된 직후(compaction)든 첫 행동은 `진행.md` 읽기이고, "지난번엔 여기까지: … 다음은 …이에요"라고 말합니다. `CLAUDE.md`/`AGENTS.md`에 덧붙은 한 줄이 이걸 다시 시키므로, 학습자는 맥락이 사라졌다는 사실조차 알 필요가 없습니다.

막히면 `진행.md`와 `지도.md`를 강사(또는 동료)에게 보내는 것이 탈출구입니다 — `guard.py pre-share`가 검사하는 바로 그 두 파일입니다. 출처: [`carry-on.md`](skills/build-with-me/references/carry-on.md).
...
**배포가 실패했어요.** 에이전트가 조용히 한 번 재시도하고, 그래도 안 되면 세 줄로 설명합니다: 무슨 일 / 왜 / 뭘 할지. 두 번 실패하면 "오늘은 내 컴퓨터에서 되는 버전까지 마무리할게요"로 마무리하고 재시도 지점을 `진행.md`에 적습니다.
...
**버전.** 현재 0.1.0이고, 무엇이 바뀌었는지는 [CHANGELOG.md](CHANGELOG.md)에 있습니다.
