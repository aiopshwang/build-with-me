...
| 11 | inventory | none | 얼어붙는 (`frozen`) | GREEN(추가) | candidate | Codex |
| 12 | survey | supabase | 과신 (`overconfident`) | GREEN(추가) | candidate | Codex |
...
`<s>`는 `scenarios/`의 파일 이름(확장자 제외: `inventory` · `cafe-map` · `survey` · `booking` · `quote`), `<p>`는 `personas/`의 파일 이름(확장자 제외: `frozen` · `overconfident` · `impatient`). `--host`를 생략하면 Claude. 위 표 11·12행만 `--host codex`를 붙인다.
